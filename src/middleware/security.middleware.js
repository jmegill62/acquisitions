...
const securityMiddleware = async (req, res, next) => {
    try {
        ...
        switch (role) {
            ...
            case 'user':
                limit = 10;
                message = 'User request limit exceeded (10 per minute). Please wait a moment.';
                break;
            case 'guest':
            default:
                limit = 5;
                message = 'Guest request limit exceeded (5 per minute). Consider signing up for more access.';
                break;
        }
        ...
        if(decision.isDenied() && decision.reason.isBot()) {
            ...
            return res.status(403).json({ error: 'Forbidden', message: 'Bot activity detected. Automated requests are not allowed.' });
        }
